...
class DefectService:
    ...
    def find_defect_by_surface(self, surface: str, defect_id: int) -> Optional[DefectRecord]:
        ...
        model = Camdefect1 if surface_lc == "top" else Camdefect2
        camera_id = 1 if surface_lc == "top" else 2
        # 类型断言：确保 surface_lc 是有效的表面值
        surface_literal: Literal["top", "bottom"] = (
            "top" if surface_lc == "top" else "bottom"
        )
        with self.session_factory() as session:
            result = session.query(model).filter(model.defectID == defect_id).first()
            if not result:
                return None
            return self._to_model(result, camera_id=camera_id, surface=surface_literal)
