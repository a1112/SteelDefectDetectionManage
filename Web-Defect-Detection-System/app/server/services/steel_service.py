...
class SteelService:
    ...
    def __init__(self, session_factory):
        ...
        # 内部缓存：存储已加载的 Rcvsteelprop，减少重复查询
        self._props_cache: dict[str, Rcvsteelprop] = {}
    ...
    def _load_props(self, session: Session, records: Iterable[Steelrecord]) -> dict[str, Rcvsteelprop]:
        ...
        with self._cache_lock:
            for steel_id in steel_ids:
                if steel_id in self._props_cache:
                    cached_ids.add(steel_id)
                else:
                    missing_ids.add(steel_id)

        # 只查询缺失的数据
        if missing_ids:
            props = session.query(Rcvsteelprop).filter(Rcvsteelprop.steelID.in_(missing_ids)).all()
            with self._cache_lock:
                for prop in props:
                    self._props_cache[prop.steelID] = prop
        ...
        with self._cache_lock:
            for steel_id in steel_ids:
                if steel_id in self._props_cache:
                    result[steel_id] = self._props_cache[steel_id]
        return result

    def clear_props_cache(self) -> None:
        """清除属性缓存，用于数据更新后刷新"""
        with self._cache_lock:
