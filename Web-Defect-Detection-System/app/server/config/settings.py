...
class ImageSettings(BaseModel):
    ...
    def _coerce_adjacent_order(cls, value: None | str | list[str]) -> list[str]:
        if value is None:
...
class LogSettings(BaseModel):
    ...
    def _coerce_modules(cls, value: None | dict[str, bool | dict[str, bool]]) -> dict[str, dict[str, bool]]:
        """规范化 modules 配置：将 bool 值转换为 {enabled: bool} 格式。"""
        if value is None:
            return {}
        ...
        for key, item in value.items():
            if isinstance(item, bool):
                ...
                normalized[key] = item  # type: ignore[assignment]
        return normalized
