...
from app.server.net_table import (
    get_api_list as get_net_api_list,
    load_map_config,
    load_map_payload,
    resolve_net_table_dir,
)
...
def get_api_list():
    ...
    for item in items:
        ...
        node.setdefault("state", "ready")
        node.setdefault("message", "服务运行中")
    ...
    fallback_host = (
        os.getenv("DEFECT_LINE_HOST")
        or os.getenv("BKJC_API_HOST")
        or "127.0.0.1"
    )
    try:
        fallback_port = int(os.getenv("BKJC_API_PORT", "8120"))
    except ValueError:
        fallback_port = 8120
